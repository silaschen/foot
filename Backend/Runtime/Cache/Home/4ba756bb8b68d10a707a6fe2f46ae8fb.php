...
font-size: 16px;"> Last access : 30 May 2014 &nbsp; <a href="/foot/admin.php/home/user/logout" class="btn btn-danger square-btn-adjust">Logout</a> </div>
  ...
  <a href="<?php echo U('Access/addnode');?>" class="btn pull-right">add</a>
                            ...
                            <div class="table table-bordered">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <?php if(is_array($list)): foreach($list as $key=>$list): ?><tr delid="<?php echo ($list["id"]); ?>">
                                            ...
                                            <td><?php if(count($list['child']) != '0'): ?><i class="csw icon-plus-sign" show="<?php echo ($list["id"]); ?>"></i><?php endif; echo ($list["name"]); ?></td>
                ...
                <td><?php if($list["status"] == '0'): ?><span class="label label-danger">锁定</span><?php else: ?><span class="label label-success">正常</span><?php endif; ?></td>
                                        ...
                                        <?php if(count($list['child']) != '0'): if(is_array($list["child"])): foreach($list["child"] as $key=>$child): ?><tr toggleid="<?php echo ($list["id"]); ?>" style="display: none;">
                                           
                                            <td><?php echo ($child["id"]); ?></td>
                                            <td><?php if(count($child['child']) != '0'): ?><i class="icon-plus-sign"></i><?php endif; echo ($child["name"]); ?></td>
                                            <td><?php echo ($child["title"]); ?></td>

                <td><?php if($child["status"] == '0'): ?><span class="label label-danger">锁定</span><?php else: ?><span class="label label-success">正常</span><?php endif; ?></td>

                                            <td>
                                             <a class="lesson-del btn btn-info"  lessonid="<?php echo ($child['id']); ?>"><i class=" icon-trash"></i></a>&nbsp  

                                            <a class="btn btn-info" id="edit" uid="<?php echo ($child["id"]); ?>" href="<?php echo U('Access/addnode',array('id'=>$child['id']));?>"><i class="icon-edit"></i></a>&nbsp
                                            <?php if($child["status"] == '1'): ?><a class="btn btn-info"  uid="<?php echo ($child["id"]); ?>" href="javascript:set(<?php echo ($child['id']); ?>,'0')">
                                            <i class="fa fa-lock"></i></a><?php else: ?>
                                             <a class="btn btn-info"  uid="<?php echo ($child["id"]); ?>" href="javascript:set(<?php echo ($child['id']); ?>,'1')">
                                            <i class="fa fa-unlock"></i></a><?php endif; ?>
...
<script type="text/javascript">
              ...
              $(".csw").click(function(){
                  var sid = $(this).attr("show");
                  $("tr[toggleid='"+sid+"']").toggle();
